...
class ComponentExtractor:
    ...
    def extract_all(self) -> ComplianceComponents:
        ...
        log(f"  Found: {len(rooms)} rooms, {len(setbacks)} setbacks, "
            f"{len(geometric_setbacks)} geometric setbacks, {len(openings)} openings, "
            f"{len(parking)} parking, {len(stairs)} stairs, {len(fire_safety)} fire safety, "
            f"{len(accessibility)} accessibility, {len(height_levels)} height/levels, "
